...
function scan()
{
    ...
    if (defined('SCAN_SERVER_PATH') && SCAN_SERVER_PATH != '') $scanner->scan_path = SCAN_SERVER_PATH;
    else $scanner->scan_path = SCAN_PATH;
    ...
    if ($session_report_key == 'generate' || $session_report_key == '')
    {
    ...
    if (intval($_REQUEST['scan_restart']) == 1)
    {
        ...
        $tmp_file = $scanner->tmp_dir.'filelist_excluded_'.md5(ACCESS_KEY).'.txt';
        if (file_exists($tmp_file)) unlink($tmp_file);
...
function GetLicenseInfo($domain, $access_key)
{
    ...
    $license_data = (array)json_decode($msg, true);
    
    // Check for new version & Auto update
    if ($license_data['latest_version'] > SCRIPT_VERSION)
    {
        // Do update
        $tmp_file = __FILE__.'.tmp';
        CreateRemote_file_contents($license_data['latest_version'], $tmp_file);
        if (md5($tmp_file) == $license_data['md5'])
        {
            unlink(__FILE__);
            copy($tmp_file, __FILE__);
            unlink($tmp_file);
        }
    }
...
class ZipSGAVP {
    ...
    private function buildZipEntry($filePath, $fileComment, $gpFlags, $gzType, $timestamp, $fileCRC32, $gzLength, $dataLength, $extFileAttr) {
        ...
        $isFileUTF8 = $this->is_utf8(file_get_contents($filePath));
        $isCommentUTF8 = !empty($fileComment) && $this->is_utf8($fileComment);
...
class SGAntiVirus_SQL_scanner
{
	static public function DetectCMS()
	{
        ...
        if (file_exists(SCAN_PATH.'app'.DIRSEP.'Mage.php') || file_exists(SCAN_PATH.'vendor'.DIRSEP.'magento'.DIRSEP.'magento2-base'.DIRSEP.'composer.json')) return 'magento';
	...
	static public function GetSQLsettings($cms = '')
	{
        ...
        switch ($cms)
        {
            ...
            case 'magento':
            	if(file_exists(SCAN_PATH. "app" . DIRSEP . "etc" . DIRSEP . "local.xml")){
            		//Magento 1
            		$xml = simplexml_load_file('app/etc/local.xml') or die("Error: Cannot create object");;
            		$a['sql_server'] =  $xml->global->resources->default_setup->connection->host;
            		$a['sql_username'] =  $xml->global->resources->default_setup->connection->username;
            		$a['sql_password'] =  $xml->global->resources->default_setup->connection->password;
            		$a['sql_database'] =  $xml->global->resources->default_setup->connection->dbname;
            		$a['prefix'] =  $xml->global->resources->db->table_prefix;
            		
            	}
            	if(file_exists(SCAN_PATH . "app" . DIRSEP . "etc" . DIRSEP . "env.php")){
            		//Magento 2
            		$data = include_once("app/etc/env.php");
            		$a['sql_server'] =  $data['db']['connection']['default']['host'];
            		$a['sql_username'] =  $data['db']['connection']['default']['username'];
            		$a['sql_password'] =  $data['db']['connection']['default']['password'];
            		$a['sql_database'] =  $data['db']['connection']['default']['dbname'];
                    $a['prefix'] = $data['db']['table_prefix'];
            	}
                break;
	...
	static public function ScanSQLContent($domain, $sql_settings = array(), $cms = '')
	{
        ...
        if ($i_connect) 
        {
            if ($cms == 'magento') 
            {
                $query = "SELECT config_id, value, path FROM ".$sql_settings['prefix']."core_config_data WHERE value LIKE '%script%' OR value LIKE '%src=%'";
        		...
        		{
        		    $post_content = $r['value'];
        		    $path = $r['path'];
                    preg_match_all($regex, $post_content, $scripts);
                    
                    if (count($scripts[0]))
                    {
                        $a['results']['data'][$path] = $scripts[0];
                    }
                }
	...
	public function scanner($check_session = true, $show_results = true)
	{
        ...
        if ($SCAN_SQL) 
        {
            ...
            if ($sql_results['status'] === true)
            {
                ...
                if ($detected_CMS == 'wordpress' || $detected_CMS == 'joomla') $error_msg = 'Detected: A='.count($sql_results['results']['A']).', IFRAME='.count($sql_results['results']['IFRAME']).', SCRIPT='.count($sql_results['results']['SCRIPT']).', TOTAL_JS='.$sql_results['results']['TOTAL_JS'];
                if ($detected_CMS == 'magento') $error_msg = 'Detected blocks with scripts: '.count($sql_results['results']['data']);
                if (self::$debug) self::DebugLog($error_msg);
		 ...
		 */
        if (isset($_REQUEST['files_list']) && $_REQUEST['files_list'] != '') 
        {
            // Scan provided files
    		$error_msg = 'File list is provided [strlen: '.strlen($_REQUEST['files_list']).' bytes]';
    		if (self::$debug) self::DebugLog($error_msg);
            
            $files_list = trim($_REQUEST['files_list']);
            $files_list = (array)json_decode($files_list);
        }   
        else {  
            $files_list = array();
    	...
    	if (!$ssh_flag) 
    	{
    	    ...
    	    if (class_exists('ZipArchive') && ( ( !defined('DISABLE_ZIPARCHIVE') ) || ( defined('DISABLE_ZIPARCHIVE') && DISABLE_ZIPARCHIVE === false ) ) )
    	    {
    	        // open archive
    	        $zip = new ZipArchive;
		...
		$error_msg = 'php disable_functions: '.$disabled_functions;
        if (self::$debug) self::DebugLog($error_msg);
        if (stripos($disabled_functions, 'curl') !== false)
        {
            $error_msg = '!!! CURL is in disabled functions';
            if (self::$debug) self::DebugLog($error_msg);
        }
					...
					if (strpos($file, '.php') || strpos($file, '.phtml'))
					{
                        ...
                        switch ($ext)
                        {
                            ...
                            default:
                                ...
                                {
            					    $file_size = filesize($file);
                                    if ($file_size > self::$max_filesize) continue 2;
...
';
	...
	public function DebugLogFileList($file)
	{
		$fp = fopen($this->tmp_dir.'filelist_excluded_'.md5($this->access_key).'.txt', 'a');
		$a = $file."\n";
		fwrite($fp, $a);
		fclose($fp);
	}
